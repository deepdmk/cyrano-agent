...
actor KyutaiPocketModelManager {
    ...
    private func copyModelsFromBundle() async -> Bool {
        ...
        // Check Models/PocketTTS/ first, then Models/ as fallback
        var bundleModelDir = bundleURL.appendingPathComponent("Models/PocketTTS")
        if !fm.fileExists(atPath: bundleModelDir.path) {
            bundleModelDir = bundleURL.appendingPathComponent("Models")
        }
